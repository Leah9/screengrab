...
class App(tk.Tk):
    ...
    def draw_screen(self):
        ...
        # Define the buttons
        capture_button = tk.Button(self, text='Capture', borderwidth=self.default_border_width, bg=self.default_button_bg, width=self.default_width, pady=self.default_pady, command=lambda: self.capture_button_clicked())
        # Pack the button, required.
        capture_button.pack(pady=2)
        get_box_button = tk.Button(self, text='Get Box', borderwidth=self.default_border_width, bg=self.default_button_bg, width=self.default_width, pady=self.default_pady, command=lambda: self.get_box_button_clicked())
        get_box_button.pack(pady=2)
        auto_button = tk.Button(self, text='Auto mode, down arrow', borderwidth=self.default_border_width, bg=self.default_button_bg, width=self.default_width, pady=self.default_pady, command=lambda : self.auto_button_clicked())
        auto_button.pack(pady=2)
        create_pdf_button = tk.Button(self, text='Create pdf', borderwidth=self.default_border_width, bg=self.default_button_bg, width=self.default_width, pady=self.default_pady, command=lambda : self.create_pdf_button_clicked())
        create_pdf_button.pack(pady=2)
        message.pack(pady=2)

        # Text box for number of pages
        pages_entry_box = tk.Entry(self, justify=tk.CENTER, borderwidth=self.default_border_width, width=self.default_width, textvariable = self.pages)
